...
def gating_mechanism(state: State):
    ...
    if response.lower() in valid:
        ...
        return Command(  # interrupt and wait for user input
...
def trading_pal():
    ...
    graph_builder.add_edge("summary", "chatbot")

    checkpointer = MemorySaver()
    return graph_builder.compile(checkpointer=checkpointer, interrupt_before=["chatbot", "summary"])
